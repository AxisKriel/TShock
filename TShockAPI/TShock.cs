					...
					if (Main.ServerSideCharacter && !player.IsLoggedIn)
					{
						...
						else if (Itembans.ItemIsBanned(player.TPlayer.inventory[player.TPlayer.selectedItem].Name, player))
						{
							...
							if (ban.AllowedGroups.Count == 0)
								s = $"You are holding a banned item: {itemName}.";
							else
								s = $"You must be at least {TShock.Groups.GetGroupByName(ban.AllowedGroups[0])} to use {itemName}.";
							player.Disable($"holding banned item: {itemName}", flags);
